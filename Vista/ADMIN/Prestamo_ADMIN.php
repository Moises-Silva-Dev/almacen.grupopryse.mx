    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <a href="INSERT/Insert_Prestamo_ADMIN.php" class="btn btn-primary">
                    <i class="fas fa-user-plus me-1"></i> Nuevo Prestamo
                </a>
                </div>
            </div>
        </div>
                        ...
                        <div class="col-md-8 mb-3 mb-md-0">
                            <form method="GET" action="" class="search-form">
                                <div class="input-group input-group-lg">
                                    ...
                                    <input type="text" class="form-control form-control-lg border-navy" name="search" placeholder="Buscar por identificador..." value="<?php echo htmlspecialchars($search ?? ''); ?>" aria-label="Buscar usuarios">
                                    <?php if (empty($search)): ?>
                            ...
                            <tbody>
                                <?php
                                    try {
                                        ...
                                        $sql = "SELECT PE.IdPrestamoE, DATE(PE.FchCreacion) AS Fecha, PE.Justificacion, PE.Estatus
                                            FROM 
                                                PrestamoE PE
                                            INNER JOIN 
                                        ...
                                        if (!empty($search)) {
                                            $searchTerm = "%$search%";
                                            $sql .= " AND (PE.IdPrestamoE LIKE ?) AND (U.Correo_Electronico = ?)";
                                        }
                                        ...
                                        if (!empty($search)) {
                                            $sql_total .= " AND (PE.IdPrestamoE LIKE ?) AND (U.Correo_Electronico = ?)";
                                        }
                                        ...
                                        if (!empty($search)) {
                                            $searchTerm = "%$search%";
                                            $stmt->bind_param("sssii", $usuario, $searchTerm, $usuario, $records_per_page, $offset);
                                        } else {
                                        ...
                                        if (!empty($search)) {
                                            $stmt_total->bind_param("sss", $usuario, $searchTerm, $usuario);
                                        } else {
                                ...
                                <tr class="border-bottom border-light">
                                    ...
                                    <td class="py-3 px-4">
                                        <div class="d-flex justify-content-center gap-2">
                                            <!-- Botón Editar -->
                                            <button class="btn btn-sm btn-outline-navy" onclick="updatePrestamoAdministrador(<?php echo $row['IdPrestamoE']; ?>)" title="Editar producto">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <!-- Botón Eliminar -->
                                            <button class="btn btn-sm btn-outline-danger" href="javascript:void(0);" onclick="eliminarRegistroPrestamo(<?php echo $row['IdPrestamoE']; ?>)">
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <small class="text-muted">
                                    ...
                                    de <strong><?php echo $total_rows; ?></strong> Borradores
                                </small>
                            </div>
