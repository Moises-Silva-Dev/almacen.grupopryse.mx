...
function InsertarNuevoBorradorRequisicionE($conexion, $id_Usuario, $FchCreacion, $Supervisor, $ID_Cuenta, $Region, $CentroTrabajo, $NroElementos, $Estado, $Receptor, $TelReceptor, $RfcReceptor, $Justificacion) {
...
function ActualizarBorradorRequisicionE($conexion, $FchCreacion, $Estatus, $Supervisor, $ID_Cuenta, $Region, $CentroTrabajo, $NroElementos, $Estado, $Receptor, $TelReceptor, $RfcReceptor, $Justificacion, $id_RequisionE) {
...
function ActualizarDomicilioBorradorRequisicionE($conexion, $Mpio, $Colonia, $Calle, $Nro, $CP, $ID_RequisionE) {
...
function SeleccionarInformacionBorradorRequisicionE($conexion, $BIDRequisicionE) {
...
function InsertarBorradorRequisicionEProductosPendientes($conexion, $FchEnvio, $InformacionRequisicionE) {
    ...
    // Verifica si el número de columnas y valores coinciden
    $StmtInsertarBorradorRequisicionEProductosPendientes = $conexion->prepare($SetenciaInsertarBorradorRequisicionEProductosPendientes);

    // Vincula los parámetros, asegurándote de que el número de parámetros coincida con el número de columnas
    $StmtInsertarBorradorRequisicionEProductosPendientes->bind_param(
        "isssiisssssissssss",
        $InformacionRequisicionE['IdUsuario'], $FchEnvio, $estatus, $InformacionRequisicionE['Supervisor'], 
        $InformacionRequisicionE['IdCuenta'], $InformacionRequisicionE['IdRegion'], $InformacionRequisicionE['CentroTrabajo'], 
        $InformacionRequisicionE['NroElementos'], $InformacionRequisicionE['Receptor'], $InformacionRequisicionE['TelReceptor'], 
    ...
    // Ejecutar la consulta
    if ($StmtInsertarBorradorRequisicionEProductosPendientes->execute()){
        return $conexion->insert_id; // Regresar true si la inserción fue exitosa
    } else {
        // Lanzar una excepción para activar el bloque catch
        throw new Exception("Error en la ejecución de la consulta de inserción.");
