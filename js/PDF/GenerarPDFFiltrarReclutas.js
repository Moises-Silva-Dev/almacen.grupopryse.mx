// Este script JavaScript se encarga de generar un PDF un filtro basado en la opciòn seleccionada en un formulario.
// Una vez que se envíe la opciòn al script PHP que genera el PDF, el PDF se muestra en un visor dentro de una ventana modal.
function Generar_PDF_Filtro_Reclutas(event) {
    ...
    var url = '../../Controlador/Reportes/Reporte_Filtrar_Reclutas.php';
    ...
    .then(blob => {
        ...
        blob.text().then(text => {
            try {
                ...
                document.getElementById('pdfIframeFiltrarReclutas').src = pdfUrl;

                // Mostrar la ventana modal que contiene el visor de PDF
                var myModal = new bootstrap.Modal(document.getElementById('pdfModalFiltrarReclutas'));
