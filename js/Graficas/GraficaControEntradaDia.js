...
async function cargarDatosMes() {
    try {
        const respuesta = await fetch('../../Controlador/GET/Graficas/getGraficaEntradaXDia.php');
...
async function cargarDatosSemana() {
    try {
        const respuesta = await fetch('../../Controlador/GET/Graficas/getGraficaEntradaXSemana.php');
        const datos = await respuesta.json();
...
function actualizarGrafica(datos, titulo) {
    const ctx = document.getElementById('graficaEntradas').getContext('2d');
        ...
        options: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: function(context) {
                            return `Entradas a Almacen: ${context.raw}`;
                        }
                    }
                }
            },
            scales: {
                y: {
                    ...
                    title: {
                        display: true,
                        text: 'Número de Entradas a Almacén',
                        color: '#001F3F',
                        font: {
                            size: 14,
                            weight: '600'
...
function actualizarInfoSemana(info) {
    ...
    infoElement.innerHTML = `
        ...
        <div class="info-item">
            ...
            <span class="info-value highlight">${info.total_semana} Entradas de Almacen</span>
        </div>
    `;
}
