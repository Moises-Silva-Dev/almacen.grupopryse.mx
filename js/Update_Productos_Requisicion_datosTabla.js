...
function actualizarDatosTabla() {
    ...
    filas.forEach(function(fila) {
        ...
        // Cambiar textContent por value para obtener el valor del input
        const cant = fila.querySelector("td:nth-child(7) input").value;
...
btnAgregar.addEventListener("click", function() {
    ...
    if (filaExistente) {
        // Recopila la fila existente
        const inputCantidad = filaExistente.querySelector("td:nth-child(7) input");        
        ...
        const nuevaFila = `
            <tr>
                ...
                <td>
                    <input type="text" class="form-control" value="${cant}" onkeypress="if (event.keyCode < 48 || event.keyCode > 57) event.returnValue = false;">
                </td>
                <td>
...
// Agregar evento para actualizar datos cuando cambia un input de cantidad
tablaProductos.addEventListener("input", function(event) {
    if (event.target.matches("td:nth-child(7) input")) {
        actualizarDatosTabla();
    }
});