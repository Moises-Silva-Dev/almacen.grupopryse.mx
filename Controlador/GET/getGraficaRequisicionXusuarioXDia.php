...
try {
    ...
    $sql = "SELECT 
            ...
            WHERE 
                MONTH(RE.FchCreacion) = MONTH(NOW())
            AND 
                YEAR(RE.FchCreacion) = YEAR(NOW())
            AND
    ...
    $stmt->bind_param('s', $usuario); // Enlazar el parámetro de consulta con el valor del usuario
    ...
    $stmt->close(); // Cerrar la sentencia SQL
