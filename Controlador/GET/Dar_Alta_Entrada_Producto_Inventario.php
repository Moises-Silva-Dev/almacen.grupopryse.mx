...
require_once("../../Modelo/Funciones/Funciones_EntradaD.php");
require_once("../../Modelo/Funciones/Funciones_EntradaE.php");
...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
    ...
    try {
        ...
        $InformacionEntradaD = SeleccionarInformacionEntradaD($conexion, $idSolicitud);
        ...
        foreach ($InformacionEntradaD as $item) {
            $idProductoEntradaD = intval($item['IdProd']);
            $idtallEntradaD = intval($item['IdTalla']);
            $cantEntradaD = intval($item['Cantidad']);

            if (!insertarInventario($conexion, $idProductoEntradaD, $idtallEntradaD, $cantEntradaD)) { // Verificar si se ha insertado correctamente
                // Lanzar una excepción en caso de error en la inserción
