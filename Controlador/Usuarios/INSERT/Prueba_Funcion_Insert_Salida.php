...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        if ($requisicionesD) { // Si hay productos pendientes
            // Procesar cada producto pendiente
            $InformacionRequisicionE = SeleccionarInformacionRequisicionE($conexion, $ID_Requisicion);

            if (!$InformacionRequisicionE) { // Si no se encuentra la requisición
            ...
            if (!$ID_RequisionEProductos) { // Si no se inserta el borrador
                // Mostrar un mensaje de error
                throw new Exception("Error al insertar en la tabla borrador_requisicionE.");
            }

            // Si no se inserta el borrador
