...
require_once("../../../Modelo/Funciones/Funciones_EntradaE.php"); // Incluir funciones de entradaE
...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
    $id_EntradaE = $_GET['id']; // Obtiene el ID de la cuenta

    // Comenzar la transacción
    $conexion->begin_transaction();

    try {
        if (ActualizarEstatusEntradaE($conexion, $id_EntradaE)) { // Cambiar estatus de la requisición
